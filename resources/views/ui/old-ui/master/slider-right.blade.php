<div class="ud-rhs">
				<div class="ud-rhs-promo">
					...
					<p>Your listing show on the top of the respective category page</p> <a href="{{route('add-promote')}}">Start now</a>
				</div>
				<!--    //Total Point Section Starts-->
				<div class="ud-rhs-poin">
					<div class="ud-rhs-poin1">
						<h4>Your Wallet Cash</h4>
						<span class="count1">{{Nam('vv_users','user_id',session('user_id'),'user_points')}}</span>
					</div>
					<div class="ud-rhs-poin2">
						...
						<a href="{{route('buy-point')}}" class="cta">Buy Wallet Cash</a>
					</div>
				</div>
				...
				<div class="ud-rhs-pay">
					<div class="ud-rhs-pay-inn">
						...
						<ul>
							<li><b>Plan name : </b>{{Nam('vv_plan_type','plan_type_id',$plan,'plan_type_name')}}</li>
							<li><b>Start date : </b> {{dateFormatConverter(Nam('vv_users','user_id',session('user_id'),'user_cdt'))}}</li>
							<li><b>Expiry date : </b>{{dateSum(Nam('vv_users','user_id',session('user_id'),'user_cdt'),Nam('vv_plan_type','plan_type_id',$plan,'plan_type_duration'))}}</li>
							<li><b>Duration : </b>  {{Nam('vv_plan_type','plan_type_id',$plan,'plan_type_duration')}} month</li>
							<li><b>Remaining Days: </b> {{floor(diff(now(),Nam('vv_users','user_id',session('user_id'),'user_cdt').'6 months'))}}</li>
							<li><span class="ud-stat-pay-btn"><b>Checkout cost:</b> Rs: {{Nam('vv_plan_type','plan_type_id',$plan,'plan_type_price')}}</span>
							</li>
				...
				<div class="ud-rhs-sec-1">
					...
					<ul>
						...
						@php $type=(Nam('vv_users','user_id',session('user_id'),'user_type')=='Service provider')?'101':102; @endphp
						@php $plan=Nam('vv_users','user_id',session('user_id'),'user_plan') @endphp
						@php $notip=sub('vv_notifications','notification_user',$plan); @endphp
						@php $notifiy=sub('vv_notifications','notification_user','100') @endphp
							@foreach($notifiy as $notifi)
							<li>
							<a target="_blank" href="{{$notifi->notification_link}}">
								<div>{{$notifi->notification_message}}.</div> <span>{{$notifi->notification_cdt}}</span>
								</a>
							</li>
							@endforeach
						...
						@endforeach
						@php $notip=sub('vv_notifications','notification_user',$type); @endphp
						@foreach($notip as $noti)
						<li>
							<a target="_blank" href="{{$noti->notification_link}}">
								<h5>{{$noti->notification_title}}</h5>
